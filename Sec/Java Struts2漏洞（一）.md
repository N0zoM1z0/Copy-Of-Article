...
这是第二篇漏洞文，本文针对 Struts2 历史版本的 RCE 进行研究和记录。由于漏洞较多，调试过程写的也比较细致，因此篇幅较长，将分为多篇文章进行发布，本文是第一篇，前置信息介绍以及 S2-001/S2-003/S2-005/S2-007/S2-008/S2-009 的漏洞调试。

了解我的人都知道，我写文章，又臭又长。因为文章的写法是边学边研究编写，在研究的过程中，如果发现之前的理解和记录有错误的地方，就回去改改，这就导致了文章的通顺性上可能不是特别好，但是我的文章胜在详细和准确，一般情况下，网上的文章不会有比我说得清楚的。

总体来讲，在我眼里这依旧是一篇水文，因为只是调式记录，并没有太多的延伸和扩展，适合和我一样对漏洞原理感兴趣的人看。
...
可以说的是，甚至在我还不知道 Struts2 是什么的时候，我就听说这个框架有非常多的 RCE 漏洞，而这些漏洞层出不迭，补也补不完，本文将记录 Struts2 全版本的高危漏洞利用和攻防过程，也会保持更新。
...
![img](https://su18.org/post-images/1618878522908.png)
...
![img](https://su18.org/post-images/1618551435638.png)
...
![img](https://su18.org/post-images/1618559956356.png)

ActionContext 是 action 的上下文，也可以叫做 action 的数据中心，本质是一个 map，所有数据都存放在这里。

![img](https://su18.org/post-images/1618626572991.png)
...
| `application` `com.opensymphony.xwork2.ActionContext.application` | 当前应用 `ServletContext` 中的`attribute`                    |
...
| `session` `com.opensymphony.xwork2.ActionContext.session`    | `HttpSession` 中的`attribute`                                |
| `parameters` `com.opensymphony.xwork2.ActionContext.parameters` | 请求参数 HashMap                                             |
...
![img](https://su18.org/post-images/1618656609695.png)

`findValue()` 方法调用了 `com.opensymphony.xwork2.util.TextParseUtil#translateVariables` 来解析和处理

![img](https://su18.org/post-images/1618656961839.png)
...
![img](https://su18.org/post-images/1618883829798.png)

然后调用 `setParameters()` 方法，循环参数 Map，首先调用 `this.acceptableName(name)` 来校验参数名是否非法，在较低版本中是判断是否包含 `#,=:`

![img](https://su18.org/post-images/1618881023371.png)

在高一点的版本中是使用正则来匹配

![img](https://su18.org/post-images/1618881030286.png)
...
![img](https://su18.org/post-images/1618899904193.png)
...
![img](https://su18.org/post-images/1618984613601.png)
...
![img](https://su18.org/post-images/1618994308303.png)
...
![img](https://su18.org/post-images/1618890320157.png)
...
![img](https://su18.org/post-images/1618902801344.png)
...
![img](https://su18.org/post-images/1619053605872.png)
...
![img](https://su18.org/post-images/1619059581984.png)
...
![img](https://su18.org/post-images/1619060627165.png)
...
![img](https://su18.org/post-images/1619079764399.png)
...
![img](https://su18.org/post-images/1619071179016.png)
...
![img](https://su18.org/post-images/1619079158081.png)
...
![img](https://su18.org/post-images/1619238146350.png)

然后调用 `getValue()` 方法实际上就是 `Ognl.getValue()` 方法解析字符串。

![img](https://su18.org/post-images/1619238180644.png)
...
![img](https://su18.org/post-images/1619249850130.png)
...
![img](https://su18.org/post-images/1619250563206.png)
...
1. Struts <= 2.3.1 ParameterInterceptor 任意文件覆盖：这是一种思路的拓展，由于 acceptedParamNames 正则允许了括号，因此可以调用一些构造方法可以执行操作的类，比如使用 FileWriter 的构造方法传入文件名可以直接创建这个文件或者清空其内容：
...
1. Struts <= 2.3.17 DebuggingInterceptor RCE
...
![img](https://su18.org/post-images/1619403020411.png)

模板引入了 struts/webconsole.html ，我们也可以直接访问这个路径来访问这个页面

![img](https://su18.org/post-images/1619403118705.png)

![img](https://su18.org/post-images/1619403124927.png)
...
![img](https://su18.org/post-images/1619403339646.png)
...
![img](https://su18.org/post-images/1619418723385.png)
...
![img](https://su18.org/post-images/1619414380180.png)
...
![img](https://su18.org/post-images/1619493318321.png)
...
![img](https://su18.org/post-images/1619501507812.png)
